...
| [`packages/skill-publish-session-export`](./packages/skill-publish-session-export) | A publishing **skill** (`publish-session-export`) that inventories text-bearing content, discusses a redaction policy with the user, sanitizes a derived copy, and checks that raw session data is not exposed | The agent, when you ask it to publish, share, commit, or build a page from a session export |
