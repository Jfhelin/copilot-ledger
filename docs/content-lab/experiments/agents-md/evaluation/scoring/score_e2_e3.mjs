...
const summ = (rows, max) => {
  const by = { BARE: [], AGENTS: [], ORIG: [], INIT: [] };
  ...
  return { max, success: rows.filter((r) => r.success).length + "/" + rows.length, BARE: stat(by.BARE), AGENTS: stat(by.AGENTS), ORIG: stat(by.ORIG), INIT: stat(by.INIT) };
};
