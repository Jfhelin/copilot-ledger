...
### 1. Confirm article scope and product identities

**Decision: this article covers three environments** — Copilot CLI, Claude CLI, and
Copilot in VS Code. Claude Code in VS Code is out of scope and excluded from every
table, figure, and claim.

For every included surface, record:
...
Do not label a proxy-based Claude integration as the official Claude Code extension.

### 2. Define the baseline protocol

Record one canonical baseline manifest:
...
Use the same model, repo, commit, and prompt across environments.

Document any configuration that cannot be matched.

### 3. Normalize the first-call metric
...
Cache treatment:
Tokenization method:
Auxiliary-request treatment:
Known limitations:
```

Apply the same definition to every environment.

### 4. Capture the first main-agent request

For each environment:

- identify the first main-agent model request
- exclude title generation, routing, summarization, and auxiliary calls
- preserve the raw request
- extract system content
- extract tools
- extract messages
- extract cache fields
- record request order and timestamp

### 5. Complete component attribution
...
Do not force a component split where the capture does not expose one.

### 6. Resolve tool delivery

For each environment, establish:

- enabled tool count
- full schemas on first request
- total tool-schema tokens
- name-only/indexed tools
- grouping/search mechanisms
- schemas introduced later
- thresholds that change behavior
- auxiliary requests that affect interpretation

Save evidence from at least the first two main-agent turns.

### 7. Verify IDE-injected context

For both IDE surfaces, identify:
...
### 8. Verify skills, memory, and instructions

For each environment, document:

- supported mechanisms
- active mechanisms in baseline
- exact prompt representation
- full-body versus summary/catalog
- on-demand retrieval
- persistence across sessions
- token contribution

Correct any older claims that conflict with the captures.

### 9. Normalize cache reporting
...
- cache creation
- cache read
- logical prompt total
- source fields
- cache scope
- whether a fresh conversation still reused a shared prefix

Do not call a derived ratio a provider-reported hit rate.

### 10. Verify context limits

For each environment, record:

- nominal model context capacity
- product-specific context limit
- maximum output tokens
- thinking/reasoning budget
- known reservations
- compaction threshold or behavior

If usable remaining capacity cannot be calculated, remove that quantitative table.

### 11. Repeat structural baselines

Recommended:

- 2–3 baseline captures per CLI
- 2–3 baseline captures per IDE if practical

Purpose:

- verify the floor is stable
- identify dynamic workspace variation
- avoid selecting an auxiliary request
- verify tool catalog consistency

These are structural consistency checks, not quality benchmarks.

### 12. Generate the final visuals

Create or update:

```text
first-call-footprint.svg
first-call-composition.svg
tool-catalog-delivery.svg
ide-context-breakdown.svg
cache-categories.svg
context-layers.svg
```

Verify:

- final numbers match the normalized ledger
- captions distinguish baseline from configured captures
- accessible `<title>` and `<desc>` are present
- alt text matches the visual
- labels remain readable on mobile
- no unverified values are embedded
...
### Matched MCP on/off capture

For each harness:

- baseline MCP off
- same environment with one identical MCP server
- compare tool count, schema tokens, cache creation, and first-call total

### One instruction-file delta

For one harness:

- baseline
- baseline plus one short repository instruction file

Measure how the fixed footprint changes.

### Configured environment snapshots

Capture realistic developer setups after the clean baseline is complete.
...
## Deliberately outside this article

Do not turn Article 3 into:

- a quality ranking
- a cost leaderboard
- a broad IDE benchmark
- a claim that smaller context is always better
- a claim that larger context is always more capable
- a universal Copilot-versus-Claude conclusion

Those require repeated matched tasks and quality scoring.
...
- [x] product identities are unambiguous
- [x] official extension and proxy captures are not conflated
- [x] the baseline protocol is documented
- [x] one first-call metric is used consistently
- [x] all `?` placeholders are replaced or explicitly marked unavailable
- [x] component attribution is labeled by evidence quality
- [x] tool-delivery claims are verified from main-agent requests
- [x] IDE context is separated into product, workspace, and user configuration
- [x] skills and memory claims match current captures
- [x] cache categories are normalized
- [x] context-capacity claims are supportable or removed
- [x] baseline and configured snapshots are not mixed
- [x] all charts use final verified data
- [x] the article states clearly that structural snapshots do not rank quality
- [ ] this checklist and all TODO comments are removed *(kept while in research mode)*
