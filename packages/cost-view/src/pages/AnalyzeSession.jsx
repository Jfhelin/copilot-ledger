...
// The Copilot Ledger report viewer. This is the original standalone app logic,
// now a self-contained page so the knowledge-site shell can host it under the
// "Analyze Session" route while the canvas extension keeps embedding it raw.
//
// Load paths (unchanged from the original app):
//   1. initialExportUrl  -- fetch JSON from that URL (canvas ?export= or gallery src)
//   2. bridge loadExport  -- canvas hand-off via the extension loopback server
...
export default function AnalyzeSession({ embed = false, initialExportUrl = null }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ color: theme.text.muted, maxWidth: 560 }}>
        Upload or select a VS Code Copilot session JSON file to inspect token cost,
        context buildup, tool calls, cache behavior and setup overhead.
      </div>
      <label
        style={{
          padding: theme.space.md + "px " + theme.space.xl + "px",
